...
An important note is, to make the flux usable to our model we first re-normalize the data, then extrapolate the sulfur data points to lower energies, and interpolate all of the data for a finer resolution.
The re-normalization of the data required multiplying the measured intensity by the JEDI energy bin widths \citep{mauk2017ssr} and 4$\pi$ to obtain a flux in ions/cm$^2$/s. 
...
The ion production rates shown in Figure \ref{fig:JEDIIonProd} demonstrate that X-ray emission will occur and also the ions do not precipitate deeply enough to go below the homopause in the atmosphere of Figure \ref{fig:atm}, indicating that absorption will have minimal effects.
. shown in Figure \ref{fig:JEDIXray}.
